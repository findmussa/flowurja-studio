...
import {
  Cpu, FolderOpen, Eye, Save, ChevronDown, ChevronRight, AlertTriangle, List,
} from "lucide-react";
...
function DisabledHintPortal({ text, rect }) {
  ...
  return createPortal(
    <div ref={popRef} style={{
      ...
      border: "0.5px solid var(--bd-popover, rgba(0,0,0,0.15))", borderRadius: 10,
      padding: "8px 12px", fontSize: 11.5, color: "var(--tx-2)", lineHeight: 1.5,
      boxShadow: "0 4px 20px rgba(0,0,0,0.14)", pointerEvents: "none",
    }}>{text}</div>,
    document.body,
  );
}
...
function Field({ label, unit, children, hint, info, disabledHint }) {
...
function Toggle({ label, value, onChange, note, disabled }) {
  return (
    <div className={s.toggleRow} style={disabled ? { opacity: 0.38 } : undefined}>
      <button
...
function TurbineSchematic() {
  ...
  return (
    <svg viewBox="0 0 100 110" width="100%" height="180" style={{ display: "block" }}>
      <line x1="50" y1="54" x2="50" y2="100" style={{ stroke: "var(--tx-4)" }} strokeWidth="5" strokeLinecap="round"/>
      <line x1="38" y1="101" x2="62" y2="101" style={{ stroke: "var(--bd-strong)" }} strokeWidth="2"/>
      <rect x="42" y="48" width="16" height="8" rx="2" style={{ fill: "var(--bg-hover-md)" }}/>
      ...
      <line x1="58" y1="52" x2="70" y2="52" stroke={c} strokeWidth="1.5" strokeLinecap="round" opacity="0.55"/>
      <rect x="70" y="47" width="9" height="10" rx="2" fill={c} fillOpacity="0.15" stroke={c} strokeWidth="0.8" strokeOpacity="0.55"/>
      <rect x="79" y="45" width="12" height="13" rx="2" fill={c} fillOpacity="0.22" stroke={c} strokeWidth="0.8"/>
      <text x="85" y="51.5" fontSize="3.8" textAnchor="middle" dominantBaseline="central" fill={c} fontFamily="-apple-system,sans-serif" fontWeight="700">DLL</text>
      <text x="4" y="100" fontSize="6" style={{ fill: c }} fontFamily="-apple-system,sans-serif" opacity="0.7">ServoDyn</text>
    </svg>
  );
}
...
export default function ServoDynPanel({ onLog, project, filePathFromProject, onDirtyChange, onRegisterSave, simRunning = false }) {
  ...
  // Derived dependency flags
  const dllActive = p.PCMode === 5 || p.VSContrl === 5 || p.YCMode === 5 || p.HSSBrMode === 5 || p.AfCmode === 5;
  const dllHint   = "Set any control mode to 5 (Bladed-style DLL) to enable this parameter";
    ...
    <div className={`${s.form} ${s.tabEnterFirst}`}>
      ...
      <div className={s.grid2}>
        <Field label="Pitch control mode (PCMode)"
          info={{ param: "PCMode", desc: "Pitch control mode switch.", range: "0, 3, 4, 5", default: "0", note: "0=no active pitch control · 3=user Fortran routine · 4=Simulink/Labview · 5=Bladed-style DLL (ROSCO / DISCON)" }}>
        ...
        <Field label="Variable-speed control (VSContrl)"
          info={{ param: "VSContrl", desc: "Variable-speed torque control mode switch.", range: "0, 1, 3, 4, 5", default: "0", note: "0=no VS control · 1=simple Region-2/3 K·Ω² control · 3=user routine · 4=Simulink · 5=Bladed DLL (ROSCO)" }}>
      ...
      <div className={s.grid2}>
        <Field label="Generator efficiency (GenEff)" unit="%"
          info={{ param: "GenEff", desc: "Mechanical-to-electrical conversion efficiency.", range: "0–100 %", default: "94.4", note: "Ignored by Thevenin (GenModel=2), user-defined (GenModel=3), and DLL/Simulink controllers." }}>
        ...
        <Field label="Time to start generator (TimGenOn)" unit="s"
          info={{ param: "TimGenOn", desc: "Time at which the generator is switched on. Used only when GenTiStr = True.", range: "≥ 0 s", default: "9999.0", note: "Set to 0 for power-production DLCs." }}>
      ...
      {dllActive && (
        <>
          <SectionHead>Bladed DLL (Quick Access)</SectionHead>
          <div className={s.grid1}>
            <Field label="DLL library path (DLL_FileName)"
              info="Path to the controller shared library (.dylib / .dll / .so). Can be absolute or relative to the run directory.">
              <div className={s.fileRow}>
                ...
                <button className={s.browseBtn} type="button"
                  onClick={async () => {
                    const f = await openDialog({ multiple: false, filters: [{ name: "DLL/SO", extensions: ["dll","so","dylib","*"] }] });
            ...
            <Field label="DLL input file (DLL_InFile)" hint="ROSCO DISCON.IN controller configuration file">
              <div className={s.fileRow}>
  ...
  const renderPitch = () => (
    <div className={`${s.form} ${s.tabEnter}`} style={{ "--tab-dir": tabDirRef.current }}>
      ...
      <div className={s.grid2}>
        ...
        <Field label="Active pitch time (TPCOn)" unit="s"
          disabledHint={p.PCMode === 0 ? "Enable a pitch control mode (PCMode ≠ 0) to use this parameter" : undefined}>
          <input className={s.inp} value={p.TPCOn} disabled={p.PCMode === 0}
      ...
      <div className={s.grid2}>
        ...
        <SelField label="Generator model (GenModel)" value={p.GenModel} onChange={v => set("GenModel", v)}
          ...
          disabledHint={p.VSContrl !== 0 ? "GenModel is only used when VSContrl = 0 (no variable-speed control)" : undefined} />
        <Field label="Generator efficiency (GenEff)" unit="%"
          disabledHint={
            (p.VSContrl === 0 && p.GenModel >= 2) ? "Ignored by Thevenin and user-defined generator models" :
            p.VSContrl >= 3 ? "Ignored when VSContrl = 3/4/5 (user-routine, Simulink, or DLL handles efficiency)" :
            undefined
          }>
          <input className={s.inp} value={p.GenEff}
            disabled={(p.VSContrl === 0 && p.GenModel >= 2) || p.VSContrl >= 3}
            onChange={e => set("GenEff", parseFloat(e.target.value) || p.GenEff)} />
        </Field>
      </div>
      ...
      <div className={s.toggleGrid}>
        <Toggle label="Timed generator start (GenTiStr)" value={p.GenTiStr} onChange={v => set("GenTiStr", v)}
          note={p.GenTiStr ? "Using TimGenOn (timed start)" : "Using SpdGenOn (speed-based start)"} />
        <Toggle label="Timed generator stop (GenTiStp)" value={p.GenTiStp} onChange={v => set("GenTiStp", v)}
          note={p.GenTiStp ? "Using TimGenOf (timed stop)" : "Stops when generator power = 0"} />
      </div>

      <div className={s.grid2}>
        <Field label="Speed to start gen. (SpdGenOn)" unit="rpm"
          disabledHint={p.GenTiStr ? "Set GenTiStr = False (speed-based start) to use this parameter" : undefined}>
          <input className={s.inp} value={p.SpdGenOn} disabled={p.GenTiStr}
            onChange={e => set("SpdGenOn", parseFloat(e.target.value) || p.SpdGenOn)} />
        </Field>
        <Field label="Time to start gen. (TimGenOn)" unit="s"
          disabledHint={!p.GenTiStr ? "Set GenTiStr = True (timed start) to use this parameter" : undefined}>
          <input className={s.inp} value={p.TimGenOn} disabled={!p.GenTiStr}
            onChange={e => set("TimGenOn", parseFloat(e.target.value) || p.TimGenOn)} />
        </Field>
        <Field label="Time to stop gen. (TimGenOf)" unit="s"
          disabledHint={!p.GenTiStp ? "Set GenTiStp = True (timed stop) to use this parameter" : undefined}>
          <input className={s.inp} value={p.TimGenOf} disabled={!p.GenTiStp}
  ...
  const renderModels = () => {
    const vsActive  = p.VSContrl === 1;
    const sigActive = p.VSContrl === 0 && p.GenModel === 1;
    const tecActive = p.VSContrl === 0 && p.GenModel === 2;
    const vsHint  = "Set VSContrl = 1 (Simple Variable-Speed) to activate these parameters";
    const sigHint = "Set VSContrl = 0 and GenModel = 1 (Simple Induction) to activate these";
    const tecHint = "Set VSContrl = 0 and GenModel = 2 (Thevenin) to activate these parameters";

    return (
      <div className={`${s.form} ${s.tabEnter}`} style={{ "--tab-dir": tabDirRef.current }}>
        <div className={s.callout}>
          Active: <strong style={{ marginLeft: 4 }}>{vsCtrlName}</strong> torque control
          {p.VSContrl === 0 && <>, <strong>{genModelName}</strong> generator model</>}.
        </div>

        <SectionHead>Simple Variable-Speed Control (VSContrl = 1)</SectionHead>
        <div className={s.grid2}>
          {[
            ...
            <Field key={k} label={lbl} unit={unit} disabledHint={!vsActive ? vsHint : undefined}>
              <input className={s.inp} value={p[k]} disabled={!vsActive}
                onChange={e => set(k, parseFloat(e.target.value) || p[k])} />
            </Field>
          ))}
        </div>

        <SectionHead>Simple Induction Generator (VSContrl = 0, GenModel = 1)</SectionHead>
        <div className={s.grid2}>
          {[
            ["SIG_SlPc","Rated slip percentage","%"],
            ["SIG_SySp","Synchronous (zero-torque) speed","rpm"],
            ["SIG_RtTq","Rated torque","N·m"],
            ["SIG_PORt","Pull-out ratio (Tpullout / Trated)","–"],
          ].map(([k, lbl, unit]) => (
            <Field key={k} label={lbl} unit={unit} disabledHint={!sigActive ? sigHint : undefined}>
              <input className={s.inp} value={p[k]} disabled={!sigActive}
                onChange={e => set(k, parseFloat(e.target.value) || p[k])} />
            </Field>
          ))}
        </div>

        <SectionHead>Thevenin-Equivalent Induction Generator (VSContrl = 0, GenModel = 2)</SectionHead>
        <div className={s.grid2}>
          {[
            ["TEC_Freq","Line frequency (50 or 60)","Hz"],
            ...
            <Field key={k} label={lbl} unit={unit} disabledHint={!tecActive ? tecHint : undefined}>
              <input className={s.inp} value={p[k]} disabled={!tecActive}
                onChange={e => set(k, parseFloat(e.target.value) || p[k])} />
            </Field>
          ))}
        </div>
        ...
        <div className={s.grid2}>
          ...
          <Field label="Brake deployment time (THSSBrDp)" unit="s"
            hint="Time at which HSS brake deployment begins">
            <input className={s.inp} value={p.THSSBrDp}
              onChange={e => set("THSSBrDp", parseFloat(e.target.value) || p.THSSBrDp)} />
          </Field>
          <Field label="Deployment duration (HSSBrDT)" unit="s"
            disabledHint={p.HSSBrMode !== 1 ? "Set HSSBrMode = 1 (Simple brake) to use this parameter" : undefined}>
            <input className={s.inp} value={p.HSSBrDT} disabled={p.HSSBrMode !== 1}
  ...
  const renderYaw = () => (
    <div className={`${s.form} ${s.tabEnter}`} style={{ "--tab-dir": tabDirRef.current }}>
      ...
      <div className={s.grid2}>
        ...
        <Field label="Yaw enable time (TYCOn)" unit="s"
          disabledHint={p.YCMode === 0 ? "Enable a yaw control mode (YCMode ≠ 0) to use this parameter" : undefined}>
          <input className={s.inp} value={p.TYCOn} disabled={p.YCMode === 0}
            onChange={e => set("TYCOn", parseFloat(e.target.value) || p.TYCOn)} />
        </Field>
        <Field label="Neutral yaw position (YawNeut)" unit="deg"
          hint="Yaw spring force is zero at this angle">
          <input className={s.inp} value={p.YawNeut}
            onChange={e => set("YawNeut", parseFloat(e.target.value) || 0)} />
        </Field>
        <Field label="Yaw spring constant (YawSpr)" unit="N·m/rad">
          <input className={s.inp} value={p.YawSpr}
            onChange={e => set("YawSpr", e.target.value)} />
        </Field>
        <Field label="Yaw damping constant (YawDamp)" unit="N·m/(rad/s)">
      ...
      <div className={s.grid2}>
        ...
        <Field label="Mean level (AfC_Mean)"
          disabledHint={p.AfCmode !== 1 ? "Set AfCmode = 1 (cosine wave) to use this parameter" : undefined}>
          <input className={s.inp} value={p.AfC_Mean} disabled={p.AfCmode !== 1}
            onChange={e => set("AfC_Mean", parseFloat(e.target.value) || 0)} />
        </Field>
        <Field label="Amplitude (AfC_Amp)"
          disabledHint={p.AfCmode !== 1 ? "Set AfCmode = 1 (cosine wave) to use this parameter" : undefined}>
          <input className={s.inp} value={p.AfC_Amp} disabled={p.AfCmode !== 1}
            onChange={e => set("AfC_Amp", parseFloat(e.target.value) || 0)} />
        </Field>
        <Field label="Phase offset (AfC_Phase)" unit="deg"
          hint="Relative to blade azimuth (0 = vertical)"
          disabledHint={p.AfCmode !== 1 ? "Set AfCmode = 1 (cosine wave) to use this parameter" : undefined}>
          <input className={s.inp} value={p.AfC_Phase} disabled={p.AfCmode !== 1}
      ...
      <div className={s.grid2}>
        {[
          ["NumBStC","BStCfiles","Blade"],
          ["NumNStC","NStCfiles","Nacelle"],
          ["NumTStC","TStCfiles","Tower"],
          ["NumSStC","SStCfiles","Substructure"],
        ].map(([numK, fileK, label]) => (
          <div key={numK} style={{ display:"flex", flexDirection:"column", gap:5 }}>
            ...
            <Field label={`${label} controller file (${fileK})`}
              disabledHint={p[numK] === 0 ? `Set ${numK} > 0 to specify a controller file` : undefined}>
              <div className={s.fileRow}>
                <input className={s.inp} value={p[fileK]} disabled={p[numK] === 0}
                  onChange={e => set(fileK, e.target.value)} />
                <button className={s.browseBtn} type="button" disabled={p[numK] === 0}
                  onClick={async () => {
                    const f = await openDialog({ multiple: false });
                    if (f) set(fileK, f);
                  }}>
  ...
  const renderInterface = () => (
    <div className={`${s.form} ${s.tabEnter}`} style={{ "--tab-dir": tabDirRef.current }}>
      {!dllActive && (
        <div className={s.callout}>
          Set PCMode, VSContrl, YCMode, HSSBrMode, or AfCmode to <strong style={{ marginLeft: 3 }}>5 (Bladed-style DLL)</strong> to enable these settings.
        </div>
      )}

      <SectionHead>Bladed-style DLL Interface</SectionHead>
      <div className={s.grid1} style={{ marginBottom: 12 }}>
        {[
          ["DLL_FileName","DLL library path (.dll / .so / .dylib)"],
          ["DLL_InFile","DLL input file (DISCON.IN)"],
          ["DLL_ProcName","Procedure name (case sensitive)"],
        ].map(([k, lbl]) => (
          <Field key={k} label={lbl} disabledHint={!dllActive ? dllHint : undefined}>
            <div className={s.fileRow}>
              <input className={s.inp} value={p[k]} disabled={!dllActive}
                onChange={e => set(k, e.target.value)} />
              {k !== "DLL_ProcName" && (
                <button className={s.browseBtn} type="button" disabled={!dllActive}
                  onClick={async () => {
                    const f = await openDialog({ multiple: false });
                    if (f) set(k, f);
                  }}>
      ...
      <div className={s.grid2}>
        <Field label="DLL time step (DLL_DT)" unit="s"
          disabledHint={!dllActive ? dllHint : undefined}>
          <input className={s.inp} value={p.DLL_DT} disabled={!dllActive}
            onChange={e => set("DLL_DT", e.target.value)} />
        </Field>
        <Field label="Blade pitch cut-off freq. (BPCutoff)" unit="Hz"
          disabledHint={!dllActive ? dllHint : undefined}>
          <input className={s.inp} value={p.BPCutoff} disabled={!dllActive}
            onChange={e => set("BPCutoff", parseFloat(e.target.value) || p.BPCutoff)} />
        </Field>
        <Field label="North yaw reference (NacYaw_North)" unit="deg"
          disabledHint={!dllActive ? dllHint : undefined}>
          <input className={s.inp} value={p.NacYaw_North} disabled={!dllActive}
      ...
      <div className={s.toggleGrid}>
        <Toggle label="Linear ramp between DLL_DT steps (DLL_Ramp)" value={p.DLL_Ramp}
          onChange={v => set("DLL_Ramp", v)} disabled={!dllActive}
          note={!dllActive ? "Requires Bladed-style DLL active" : "Introduces time shift when enabled"} />
      </div>
      ...
      <Collapsible title="Bladed DLL control records (Ptch_*, Gain_OM, GenSpd_*, etc.)"
        ...
        <div className={s.grid3}>
          {[
            ...
            ["Gain_OM","Optimal mode gain (Nm/(rad/s)²)"],
            ...
            ["DLL_NumTrq","T-S table points (0 = use opt. mode)"],
          ].map(([k, lbl]) => (
            <Field key={k} label={lbl} disabledHint={!dllActive ? dllHint : undefined}>
              <input className={s.inp} value={p[k]} disabled={!dllActive}
                onChange={e => set(k, parseFloat(e.target.value) ?? p[k])} />
  ...
  const renderOutput = () => (
    <div className={`${s.form} ${s.tabEnter}`} style={{ "--tab-dir": tabDirRef.current }}>
      ...
      <div className={s.grid2}>
        <Field label="Output file switch (OutFile)"
          info={{ param: "OutFile", desc: "Where to place output: 1 = module file, 2 = glue-code file, 3 = both.", range: "1, 2, 3", default: "1" }}>
          <select className={s.select} value={p.OutFile} onChange={e => set("OutFile", Number(e.target.value))}>
            {[{v:1,l:"1 – Module file only"},{v:2,l:"2 – Glue-code file only"},{v:3,l:"3 – Both"}]
              .map(o => <option key={o.v} value={o.v}>{o.l}</option>)}
          </select>
        </Field>
        <Field label="Output format (OutFmt)"
          info="C-style format string for tabular output. Must produce a 10-character field. Default: ES10.3E2.">
