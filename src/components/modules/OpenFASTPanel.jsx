...
function buildFstContent(p, prefix) {
  ...
  return [
    `------- OpenFAST INPUT FILE -------------------------------------------`,
    `Generated by FlowWake Studio — ${prefix}`,
    `---------------------- SIMULATION CONTROL --------------------------------------`,
        ...
        <div className={s.formArea}>
          <div className={s.form}>
            ...
            <div className={s.toggleGrid}>
              ...
              <Toggle label="Find steady-state operating point first (CalcSteady)" value={p.CalcSteady} onChange={set("CalcSteady")} info={{ param: "CalcSteady", desc: "Compute a periodic steady-state operating point before linearising.", default: "FALSE", note: "Required when rotor speed needs to be trimmed to rated. The trim routine runs before linearisation begins." }} />
            </div>
            <div className={s.grid2}>
              <Field label="Trim control (TrimCase)" info={{ param: "TrimCase", desc: "Which actuator to use during the steady-state trim.", range: "1–3", default: "3", note: "1 = yaw angle; 2 = generator torque; 3 = collective blade pitch." }}>
                <select value={p.TrimCase} onChange={setN("TrimCase")} disabled={!p.CalcSteady}>
              ...
              <Field label="Speed convergence tolerance (TrimTol)" info={{ param: "TrimTol", desc: "Rotor speed error tolerance for the trim convergence criterion.", range: "> 0", default: "1.0E-4", note: "Smaller values give a more accurate operating point but take longer to converge." }}>
                <input type="number" value={p.TrimTol} step={0.0001} min={0} onChange={setN("TrimTol")} disabled={!p.CalcSteady} />
                <p className={s.hint}>Used only when CalcSteady = True</p>
              </Field>
              <Field label="Speed error gain (TrimGain)" info={{ param: "TrimGain", desc: "Proportional gain for the trim feedback loop.", range: "> 0", default: "0.001", note: "Increase if convergence is slow; reduce if the trim oscillates." }}>
                <input type="number" value={p.TrimGain} step={0.001} min={0} onChange={setN("TrimGain")} disabled={!p.CalcSteady} />
              </Field>
              <Field label="Tower damping (Twr_Kdmp)" unit="N/(m/s)" info={{ param: "Twr_Kdmp", desc: "Artificial dashpot gain added to tower DOFs during the trim phase to suppress transients.", range: "≥ 0", unit: "N/(m/s)", default: "0.0" }}>
                <input type="number" value={p.Twr_Kdmp} step={0.1} min={0} onChange={setN("Twr_Kdmp")} disabled={!p.CalcSteady} />
              </Field>
              <Field label="Blade damping (Bld_Kdmp)" unit="N/(m/s)" info={{ param: "Bld_Kdmp", desc: "Artificial dashpot gain added to blade DOFs during the trim phase to suppress transients.", range: "≥ 0", unit: "N/(m/s)", default: "0.0" }}>
                <input type="number" value={p.Bld_Kdmp} step={0.1} min={0} onChange={setN("Bld_Kdmp")} disabled={!p.CalcSteady} />
              </Field>
            </div>
            ...
            <div className={s.grid2}>
              ...
              <Field label="Linearization times (LinTimes)" unit="s" info={{ param: "LinTimes", desc: "Explicit simulation times at which to linearise, comma-separated.", unit: "s", default: "30, 60", note: "Ignored when CalcSteady is true — azimuth angles are used instead." }}>
                <input type="text" value={p.LinTimes} onChange={setE("LinTimes")} disabled={!p.Linearize}
                  placeholder="e.g. 30, 60" />
                <p className={s.hint}>Comma-separated list; ignored when CalcSteady = True</p>
              </Field>
            </div>
            ...
            <div className={s.toggleGrid}>
              <Toggle label="Write full Jacobians to linearization file (LinOutJac)" value={p.LinOutJac} onChange={set("LinOutJac")} info={{ param: "LinOutJac", desc: "Append the full input/output Jacobian matrices to the .lin file.", default: "FALSE", note: "Creates large files; mainly useful for debugging module coupling." }} />
              <Toggle label="Write module-level linearization files (LinOutMod)" value={p.LinOutMod} onChange={set("LinOutMod")} info={{ param: "LinOutMod", desc: "Write separate .lin files for each submodule (ElastoDyn, AeroDyn, etc.) in addition to the combined system file.", default: "FALSE" }} />
            </div>
            ...
            <div className={s.toggleGrid}>
              <Toggle label="Write mesh fields to VTK files (VTK_fields)" value={p.VTK_fields} onChange={set("VTK_fields")} info={{ param: "VTK_fields", desc: "Additionally write mesh field data (velocities, accelerations, forces) to VTK files.", default: "FALSE", note: "Significantly increases output file size." }} />
            </div>
          </div>
        </div>
      )}
